...
- First, Yishun does land in the lower half of the price distribution across all flat types. 
...
- $C(\text{town}_i)$ is a set of dummy variables, one per town, that absorb the location premium or discount after controlling for everything else. 
- $C(\text{flat\_type}_i)$ removes the mechanical effect of comparing 3-room prices to 5-room prices. 
- $\beta\,\text{flat\_age}_i$ captures the depreciation gradient common across HDB stock: older flats trade at a discount because of shorter remaining lease tenure and higher maintenance costs. 
- $C(\text{year\_month}_i)$ absorbs the macro market cycle, so the town coefficient is not contaminated by periods when Yishun happened to sell more units.
...
The coefficient to focus on is the **Yishun town dummy**. Econometrically, that coefficient is the average gap between Yishun and the omitted reference town **after** matching transactions on flat type, flat age, and transaction month. 
- A negative coefficient means Yishun still trades cheaper even when we compare like with like; 
- A coefficient close to zero would mean the apparent affordability was mostly compositional; 
- A positive coefficient would mean Yishun is actually more expensive once controls are applied. 
In other words, the town dummy is our estimate of the residual location effect.
...
The baseline model imposes a single Yishun coefficient regardless of flat type. If the discount varies by segment, for example because 5-room buyers are disproportionately priced out of better-located towns while 3-room buyers have more options, then a pooled estimate will mask strategically useful information.
...
The interpretation changes slightly in this model. The main town coefficient now captures Yishun's discount for the **reference flat type**, while each interaction term measures how much the Yishun discount changes when we move from that reference category to another flat type. This is why the output is more informative than a single pooled coefficient: it tells us whether the affordability story is concentrated in certain buyer segments rather than being uniform across the town's housing stock.
...
This gradient reveals that Yishun's value proposition is not uniform: 
- it is most pronounced in the larger flat segments, where buyers face higher absolute prices elsewhere and are 
therefore more sensitive to town discounts. 
  - The 3-room discount, at −11.5%, is real but modest; 
  - The 5-room discount, at −30.1%, is substantial. 
- This heterogeneity has direct implications for how Yishun should be positioned in any market communication. A blanket "affordable town" message leaves the most compelling part of the story on the table.
